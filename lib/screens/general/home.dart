...
class _HomePageState extends State<HomePage> {
  ...
  void SendForumMessage(Map user, message)async{
    ...
    http.Response response =await http.post(Uri.parse(base_url +"general/send-message-to-forum/"), body: _data);
    if(response.statusCode< 206){
  ...
  void getForumMessageds()async{
    List? _data;
    final userId = await UserLocalData.userID();
    http.Response response = await http.get(Uri.parse(base_url + "general/retrieve-forum-messages/$userId/"));
  ...
  void getIndieMessageds()async{
    List? _data;
    final userId = await UserLocalData.userID();
    http.Response response = await http.get(Uri.parse(base_url + "constituent-operations/retrieve-messages/$userId/"));
                  ...
                  Row(
                    ...
                    children: [
                      TextButton(
                          ...
                          onPressed: () {
                            Navigator.push(context, MaterialPageRoute(builder: (context)=>const CreateProject()));
                          },
                          child: const Text(
                            "Create Project",
          ...
          child: Padding(
            ...
            child: Column(
              children:  [
                ...
                if(_pro.allConstituents != null && _pro.allConstituents!.isNotEmpty)
                for(var con in _pro.allConstituents!)
                Padding(
                  padding: const EdgeInsets.only(bottom: 10.0),
                  child: InkWell(
                    onTap: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>ChatMP(conId: con['system_id_for_user'],con: con,))),
                    child: Row(
                      children:  [
                         ...
                         Column(
                           ...
                           children: [
                             Text("${con['full_name']}", style: const TextStyle(fontSize: 13),),
                             Text("${con['email']}", style: const TextStyle(fontSize: 11),),
                      ...
                      if(_forumMessages.isNotEmpty)
                        for (var data in _forumMessages)
                          Column(
                            ...
                            children: [
                              Row(
                                mainAxisAlignment: data['userId'] != data['sender']['system_id_for_user'] ? MainAxisAlignment.start: MainAxisAlignment.end,
                                children: [
                                  if(data['sender']['system_id_for_user'] == mpId)
                                    const Icon(Icons.check_circle, color: appColor, size: 20,),
                                const SizedBox(width: 3,),
                                Text("${data['sender']['full_name']}", style: const TextStyle(
                                    color: Colors.black,
        ...
        : Stack(
            children: [
              Scaffold(
                ...
                floatingActionButton: _pro.userData!['is_mp'] && _selectedIndex == 0
